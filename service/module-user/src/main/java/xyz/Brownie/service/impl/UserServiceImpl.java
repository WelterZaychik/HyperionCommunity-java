...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    @Override
    @Transactional
    public void adjustFollowsCount(Long userId, int delta) {
        if (delta == 0) return;

        lambdaUpdate()
                .setSql(delta > 0 ? "follows_count = follows_count + " + delta :
                        "follows_count = follows_count - " + (-delta))
    ...
    @Override
    @Transactional
    public void adjustFansCount(Long userId, int delta) {
        if (delta == 0) return;

        lambdaUpdate()
                .setSql(delta > 0 ? "fans_count = fans_count + " + delta :
                        "fans_count = fans_count - " + (-delta))
                .eq(User::getId, userId)
                .update();
    }
}
