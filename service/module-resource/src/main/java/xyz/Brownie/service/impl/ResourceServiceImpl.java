...
public class ResourceServiceImpl extends ServiceImpl<ResourceMapper, Resource>
    ...
    public Result AddResource(Resource resource) {
        ...
        try {
            if (!StringUtils.hasText(resource.getTitle())){
                ...
                resNo.put("msg",msg);
                throw new EmptyContentException(msg);
            } else if(resource.getIsDelete()==null){
                msg = "是否删除为空";
                resNo.put("msg",msg);
                throw new EmptyContentException(msg);
    ...
    @Override
    public Result getResouces(long tagId) {
        resYes = new HashMap();
        if (tagId != 0){
            LambdaQueryWrapper<Resource> wrapper = new LambdaQueryWrapper<>();
            wrapper.eq(Resource::getTagflag,tagId);
            List<Resource> ResourceListByTagId = list(wrapper);
            resYes.put("ResouecesList",ResourceListByTagId);
        }else {
            List<Resource> ResourceListByTagId = list();
            resYes.put("ResouecesList",ResourceListByTagId);
        }
        return Result.suc(ResponseCode.Code200,resYes);
    }
