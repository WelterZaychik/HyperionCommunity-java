...
public class TopicController {
    ...
    public Result hostoftopic(@PathVariable("curPage") int curPage , @PathVariable("pageSize") int pageSize){
        return topicService.HostOfTopic(curPage,pageSize);
    }
    //双重排序
    @GetMapping("/SelectChoose/{current}/{pageSize}")
    public Result SelectChoose(@PathVariable("current") Integer current, @PathVariable("pageSize") Integer pageSize){
        return topicService.SelectChoose(current,pageSize);
    }
